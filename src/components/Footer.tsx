...
export default function Footer() {
  return (
    // 👇 Background and main text color changed here 👇
    <footer className="bg-neutral-700 text-white py-16 px-6 md:px-12">
        ...
        <div>
          <Link href="/">
            <Image
              ...
              alt="Shamika Thiranjana Photography Logo"
              width={150}
              height={60}
          ...
          {/* Inherits text-white */}
          <p className="text-sm opacity-90">Preserving timeless moments with passion and artistry. Based in Sri Lanka, available for destination weddings worldwide.</p>
        </div>

        {/* Column 2: Navigation Links */}
        {/* 👇 Link hover colors changed 👇 */}
        <nav className="flex flex-col space-y-3 text-sm items-center md:items-start">
          <Link href="/about" className="hover:opacity-80 transition-opacity">About</Link>
          <Link href="/albums" className="hover:opacity-80 transition-opacity">Collection</Link>
          <Link href="/articles" className="hover:opacity-80 transition-opacity">Articles</Link>
        </nav>

        {/* Column 3: Contact Info and Social Media */}
        <div>
          {/* 👇 Text colors changed 👇 */}
          <p className="font-semibold text-white">CONTACT@SHAMIKAPHOTOGRAPHY.COM</p>
          <p className="text-2xl font-semibold text-white my-2">+94 77 4684941</p>

          {/* Social Media Icons */}
           {/* 👇 Link hover colors changed 👇 */}
          <div className="flex justify-center md:justify-start space-x-4 mt-4">
            <a href="https://www.facebook.com/your-profile-url" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="text-white/80 hover:text-white transition-colors">
            ...
            <a href="https://www.instagram.com/your_username" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-white/80 hover:text-white transition-colors">
            ...
            <a href="https://wa.me/94774684941" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="text-white/80 hover:text-white transition-colors">
      ...
      {/* 👇 Border and text colors adjusted 👇 */}
      <div className="mt-12 pt-8 border-t border-white/20 text-center text-sm">
        ...
        <p className="mt-2 text-xs text-white/70">Welcome to shamikathiranjanaphotography.lk</p>
        <p className="mt-4 text-xs text-white/50">
          <a
            href="https://nund.dev"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-white/80 transition-colors tracking-wider"
          >
            WEBSITE BY NUND
          </a>
        </p>
